...
# Unique genes, 16981
uniGenes <- intersect(names(which(getGeneLengths == 1)), rownames(data.norm))
length(uniGenes)
...
getMapping <- sapply(rownames(data.norm), function(x) {
...
# 1930 genes in our data are not distinguishable in Halpern's data:
...
geneSet <- data.frame(MortenGene = c(getMapping$GENE, uniGenes), HalpernGene = c(getMapping$MULTIGENE, uniGenes), stringsAsFactors=F)
head(geneSet)

# These are the -unique- genes to use in each dataset:
useMorten.genes <- unique(c(getMapping$GENE, uniGenes))
